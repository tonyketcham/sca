...
export function cloneSimulationState(state: SimulationState): SimulationState {
  return {
    ...
    obstacles: state.obstacles.map((polygon) =>
...
function createSeedNodes(
  ...
  if (params.seedPlacement === 'scatter') {
    ...
    return Array.from({ length: count }, (_, index) => ({
      x: clamp(
        centerX + (rng() - 0.5) * spreadX,
        margin,
        bounds.width - margin,
      ),
      y: clamp(
        centerY + (rng() - 0.5) * spreadY,
        margin,
  ...
  const spread =
    (isHorizontalEdge(params.seedEdge) ? bounds.width : bounds.height) *
    spreadPercent;
...
function getSeedEdgeCenter(
  bounds: Bounds,
  margin: number,
  edge: SimulationParams['seedEdge'],
): Vec2 {
  switch (edge) {
    case 'left':
