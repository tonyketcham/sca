...
const migrations: Record<number, Migration> = {
  ...
  2: (input) => ({
    ...
    params: {
      ...input.params,
      seedPlacement: isSeedPlacement(input.params.seedPlacement)
        ? input.params.seedPlacement
        : 'edge',
      seedEdge: isSeedEdge(input.params.seedEdge)
        ? input.params.seedEdge
        : 'top',
      seedAngle: Number.isFinite(input.params.seedAngle)
        ? input.params.seedAngle
        : 0,
    },
  }),
  3: (input) => ({
...
function isSeedPlacement(
  value: unknown,
): value is SimulationParams['seedPlacement'] {
...
function normalizeTemplateGrid(
  input: Partial<TemplateGridSettings> | undefined,
): TemplateGridSettings {
  return {
    ...
    showGutter:
      typeof input?.showGutter === 'boolean' ? input.showGutter : true,
    gutterAsObstacles:
      typeof input?.gutterAsObstacles === 'boolean'
        ? input.gutterAsObstacles
        : false,
  };
}
...
function normalizeFrame(
  ...
  return {
    ...
    params: {
      ...
      seedPlacement: isSeedPlacement(frame.params?.seedPlacement)
        ? frame.params!.seedPlacement
        : 'edge',
      seedEdge: isSeedEdge(frame.params?.seedEdge)
        ? frame.params!.seedEdge
        : 'top',
      ...
      pathSmoothing: clamp01(finiteNumber(frame.params?.pathSmoothing, 0)),
    },
    obstacles: {
    ...
    renderSettings: {
      showAttractors:
        typeof frame.renderSettings?.showAttractors === 'boolean'
          ? frame.renderSettings.showAttractors
          : false,
      showNodes:
        typeof frame.renderSettings?.showNodes === 'boolean'
          ? frame.renderSettings.showNodes
          : false,
      showObstacles:
        typeof frame.renderSettings?.showObstacles === 'boolean'
          ? frame.renderSettings.showObstacles
          : true,
      strokeWidth: Number(frame.renderSettings?.strokeWidth ?? 1.4),
      nodeRadius: Number(frame.renderSettings?.nodeRadius ?? 1.5),
      rootColor: String(frame.renderSettings?.rootColor ?? '#6ea95a'),
      obstacleFill: String(frame.renderSettings?.obstacleFill ?? '#756c64'),
      attractorColor: String(frame.renderSettings?.attractorColor ?? '#5c84f4'),
    },
    exportSettings: {
      ...
      durationMode:
        frame.exportSettings?.durationMode === 'auto' ? 'auto' : 'fixed',
    },
