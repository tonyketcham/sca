...
function fromCompact(input: unknown): ConfigState | null {
  ...
  if (version >= 5) {
    ...
    if (!grid || !Array.isArray(frames) || !Array.isArray(selected))
      return null;
    const parsedFrames = frames
      .map((item) =>
        Array.isArray(item) ? frameFromCompact(item as CompactConfig) : null,
      )
...
function frameToCompact(frame: FrameConfig): CompactConfig {
  return [
    [
      ...
      frame.params.seedEdge === 'bottom'
        ? 1
        : frame.params.seedEdge === 'left'
          ? 2
          : frame.params.seedEdge === 'right'
            ? 3
            : 0,
