...
import { renderComposite, renderSimulation, type GridLayout, type RenderSettings } from '../render/canvasRenderer'
...
export async function encodeCompositeMp4(options: CompositeMp4Options): Promise<Blob> {
  if (!('VideoEncoder' in window)) {
    throw new Error('WebCodecs VideoEncoder is not available in this browser.')
  }

  const width = Math.round(options.grid.cellWidth * options.grid.cols)
  const height = Math.round(options.grid.cellHeight * options.grid.rows)
  const frameCount =
    options.durationMode === 'fixed'
      ? Math.max(1, Math.floor(options.durationSeconds * options.fps))
      : 0
  ...
  const frameStates = options.frames.map((frame, index) => {
    ...
    const obstacles = source ? source.obstacles.map((polygon) => polygon.map((point) => ({ ...point }))) : []
  ...
  const autoFrames = options.frames.reduce((max, frame) => {
    const estimate = Math.ceil(frame.params.maxNodes / Math.max(1, options.stepsPerFrame)) * 4
    return Math.max(max, estimate)
  }, 0)
  const maxFrames = options.durationMode === 'auto' ? Math.max(120, autoFrames) : frameCount

  let frameIndex = 0
  while (frameIndex < maxFrames && (options.durationMode === 'fixed' || frameStates.some((state) => !state.completed))) {
